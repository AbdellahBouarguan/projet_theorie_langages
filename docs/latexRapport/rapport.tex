...
\Large Période de réalisation : 23 Mars 2026 - 28 Mars 2026 \\
\HRule \\
\vspace*{1cm}
\end{center}

\section{Introduction de la Phase 3}
Cette troisième phase marque le point culminant de notre moteur d'automates. L'enjeu est de passer de simples automates statiques à des structures dynamiques capables d'être combinées et transformées. Nous avons introduit les opérations de base (concaténation, union) et l'algorithme de \textbf{Thompson} pour convertir des expressions régulières en NFAs. Un point critique de cette phase est la gestion des transitions $\epsilon$ (epsilon), indispensables pour l'algorithme de Thompson, mais qui nécessitent une suppression rigoureuse (\textbf{fermeture-epsilon}) pour simplifier l'analyse et l'affichage final.

\section{Détails Algorithmiques}

\subsection{Réindexation des états (méthode d'Offset)}
Lors de la concaténation ou de l'union de deux automates $A$ et $B$, un problème majeur survient : la collision des identifiants (IDs) d'états. Si $A$ possède un état $0$ et $B$ également, la fusion directe corrompt la logique du graphe.
\textit{Solution :} Nous avons implémenté une fonction utilitaire \texttt{copy\_and\_offset\_states}. Avant d'ajouter $B$ au nouvel automate, nous calculons le plus grand ID présent dans $A$ et l'utilisons comme décalage (\textit{offset}) pour tous les IDs de $B$. Cette approche garantit une séparation stricte des espaces de noms d'états dans l'automate résultant.

\subsection{Gestion du non-déterminisme et transitions $\epsilon$}
Deux approches ont été explorées pour gérer les transitions vides ($\epsilon$) :
\begin{enumerate}
    \item \textbf{Backtracking (Test de mots)} : Lors de la simulation, l'algorithme explore récursivement tous les chemins possibles dès qu'il rencontre une transition $\epsilon$, sans consommer de caractère. Bien qu'efficace pour tester ponctuellement, cette méthode devient coûteuse sur de grands graphes.
    \item \textbf{Suppression d'Epsilon ($\epsilon$-closure)} : Pour "nettoyer" l'automate, nous calculons la fermeture transitive des transitions $\epsilon$ via l'algorithme de **Floyd-Warshall**. Pour chaque chemin $q_i \xrightarrow{\epsilon^*} q_j \xrightarrow{a} q_k \xrightarrow{\epsilon^*} q_l$, nous créons une transition directe $q_i \xrightarrow{a} q_l$. Cette transformation simplifie radicalement l'automate en le ramenant à un AFN sans transitions $\epsilon$.
\end{enumerate}

\section{Code Source Significatif}

\subsection{Concaténation d'automates}
La concaténation relie chaque état final de l'automate $A$ aux états initiaux de $B$ par des transitions $\epsilon$.
...
Automaton *concatener_automates(const Automaton *a1, const Automaton *a2) {
  ...
  copy_and_offset_states(res, a1, 0); // Copie de A
  int offset = get_max_id(a1) + 1;
  copy_and_offset_states(res, a2, offset); // Copie de B decalee

  for (int i = 0; i < a1->num_etats; i++) {
    if (a1->is_final[i]) {
      res->is_final[i] = false; // Transition vers B
...
\subsection{Suppression physique des Epsilon}
Cette fonction utilise une matrice d'adjacence booléenne pour calculer la fermeture.
\begin{lstlisting}[language=C, caption=Suppression des transitions epsilon (fermeture transitive)]
void supprimer_epsilon_transitions(Automaton *a) {
  bool closure[MAX_ETATS][MAX_ETATS];
  // ... Calcul de la fermeture via Floyd-Warshall ...
  ...
  // Heritage des etats finaux
  for (int i = 0; i < a->num_etats; i++)
    for (int j = 0; j < a->num_etats; j++)
      if (closure[i][j] && a->is_final[j]) a->is_final[i] = true;

  // Generation des transitions directes sans epsilon
  // ... (Code de generation de new_transitions) ...
...
\begin{itemize}
    \item \textbf{Abdellah} : Architecture des opérations complexes et implémentation de l'algorithme de Thompson (regex vers NFA) et Shunting-Yard.
    \item \textbf{Nizar} : Conception de la logique de réindexation (\textit{offsetting}) et des outils de fusion.
